...
from utils.cache.global_variables import TESTING_GA, TESTING_LOTTERY
...
def parse_lottery_duration(duration_str: str) -> int:
    """Parses a duration string and returns total seconds."""
    # Normalize for matching
    duration_str = duration_str.lower().replace(" ", "")
    ...
    if not match:
        ...
        msg = "Invalid format. Examples: `3d`, `3 days`, `4d12h`, `4 days 12 hours`, `30m`, `1h30m`, `45s`, `1m30s`"
        return None, msg
    days = int(match.group(1)) if match.group(1) else 0
    hours = int(match.group(2)) if match.group(2) else 0
    minutes = int(match.group(3)) if match.group(3) else 0
    ...
    if total_seconds < MIN_GA_DURATION_SECONDS and not TESTING_LOTTERY:
        return None, "Lottery duration must be at least 30 minutes."

    unix_end = int(time.time() + total_seconds)
    return int(unix_end), None
